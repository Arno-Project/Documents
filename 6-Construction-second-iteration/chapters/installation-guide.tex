...
همچنین برای یکپارچه‌سازی دستورات اجرا از \lr{make} استفاده شده است که می‌توانید آن را به راحتی با کمک مدیر بسته‌ی سیستم‌عامل خود (\lr{apt} برای \lr{debian}, \lr{choco} برای \lr{Windows} و \lr{brew} برای \lr{macOS}) نصب کنید.

\section{نصب Docker}

برای نصب \lr{Docker} روی سیستم‌های لینوکسی مطابق توضیحات داده‌شده در \href{https://docs.docker.com/compose/install/compose-plugin/#install-using-the-repository}{این لینک} عمل کنید. 
...
دریافت کنید.
در پوشه‌ی \lr{backend} فایل \lr{.env.template} را کپی کرده و با نام \lr{.env} ذخیره کرده و مقادیر خالی را درون آن پر کنید.
سپس در هر مخزن به شاخه‌ای که \lr{Makefile} در آن قرار دارد رفته و دستور \lr{make build} را اجرا کنید.
